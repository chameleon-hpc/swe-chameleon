...
#include "blocks/SWE_DimensionalSplittingStarPU.hh"
...
int main(int argc, char** argv) {
	...
	starpu_mpi_init_comm(&argc, &argv, 1, MPI_COMM_WORLD);
    starpu_mpi_comm_rank(MPI_COMM_WORLD, &myRank);
    starpu_mpi_comm_size(MPI_COMM_WORLD, &numRanks);
	...
	SWE_DimensionalSplittingStarPU* blocks[xBlockCount][yBlockCount];
	...
	for(int x = xBounds[myXRank]; x < xBounds[myXRank+1]; x++) {
		for(int y = yBounds[myYRank]; y < yBounds[myYRank+1]; y++) {
			...
			blocks[x][y] = new SWE_DimensionalSplittingStarPU(x_blocksize, y_blocksize, dxSimulation, dySimulation, originX, originY);
	...
	SWE_DimensionalSplittingStarPU writeBlock(nxRequested, nyRequested, dxSimulation, dySimulation, 0, 0);
	BoundaryType boundaries[4];
	boundaries[BND_LEFT] = scenario.getBoundaryType(BND_LEFT);
	boundaries[BND_RIGHT] = scenario.getBoundaryType(BND_RIGHT);
	boundaries[BND_TOP] = scenario.getBoundaryType(BND_TOP);
	...
	printf("StarPU: Computation ended, walltime:%f\n", wallTime);
